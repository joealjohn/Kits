...
public final class HikariProxyConnection extends ProxyConnection implements Wrapper, AutoCloseable, Connection {
  ...
  public boolean setShardingKeyIfValid(ShardingKey paramShardingKey1, ShardingKey paramShardingKey2, int paramInt) throws SQLException {
    try {
      return this.delegate.setShardingKeyIfValid(paramShardingKey1, paramShardingKey2, paramInt);
    } catch (SQLException sQLException) {
      throw checkException(sQLException);
    } 
  }
  
  public boolean setShardingKeyIfValid(ShardingKey paramShardingKey, int paramInt) throws SQLException {
    try {
      return this.delegate.setShardingKeyIfValid(paramShardingKey, paramInt);
    } catch (SQLException sQLException) {
      throw checkException(sQLException);
    } 
  }
  
  public void setShardingKey(ShardingKey paramShardingKey1, ShardingKey paramShardingKey2) throws SQLException {
    try {
      this.delegate.setShardingKey(paramShardingKey1, paramShardingKey2);
    } catch (SQLException sQLException) {
      throw checkException(sQLException);
    } 
  }
  
  public void setShardingKey(ShardingKey paramShardingKey) throws SQLException {
    try {
      this.delegate.setShardingKey(paramShardingKey);
    } catch (SQLException sQLException) {
      throw checkException(sQLException);
    } 
  }
  
  protected HikariProxyConnection(PoolEntry paramPoolEntry, Connection paramConnection, FastList<Statement> paramFastList, ProxyLeakTask paramProxyLeakTask, boolean paramBoolean1, boolean paramBoolean2) {
    super(paramPoolEntry, paramConnection, paramFastList, paramProxyLeakTask, paramBoolean1, paramBoolean2);
  }
}


/* Location:              C:\Users\joeal\Downloads\Kits-2.0.9.jar!\dev\continuum\kits\libs\hikari\pool\HikariProxyConnection.class
